...
def _local_support_totals(player, target, planets, fleets=None, radius=ATTACK_SUPPORT_RADIUS):
    ...
    enemy = 0
    for planet in planets:
        if int(planet.id) == int(target.id):
            continue
        if distance(planet, target) > float(radius):
            continue
        weight = int(planet.ships) + 2 * int(planet.production)
        if int(planet.owner) == player:
            friendly += weight
        elif int(planet.owner) not in (player, -1):
            enemy += weight

    for fleet in fleets or []:
        if distance_xy(float(fleet.x), float(fleet.y), float(target.x), float(target.y)) > float(radius):
            continue
        if int(fleet.owner) == player:
...
def decisive_attack_margin_for_state(planets, fleets, player, source, target, ships):
    if source is None or target is None or int(ships) <= 0 or int(target.owner) == int(player):
        return 0.0

    eta = estimate_travel_eta(source, target, ships)
    production_buffer = 0 if int(target.owner) == -1 else int(math.ceil(float(target.production) * eta))
    base_need = int(target.ships) + production_buffer + 1
    friendly_support, enemy_support = _local_support_totals(player, target, planets, fleets=fleets)
    support_pressure = max(0, int(enemy_support) - int(friendly_support))
    support_buffer = min(18, int(math.ceil(float(support_pressure) / 6.0)))
    static_buffer = 2 if is_static(target) and int(target.owner) != -1 else 0
    big_buffer = 2 if size_label(target) == "big" and int(target.owner) != -1 else 0
    decisive_need = max(1, base_need + support_buffer + static_buffer + big_buffer)
    return clamp(
        (float(ships) - float(decisive_need)) / max(12.0, float(decisive_need)),
        -1.0,
        1.0,
    )
...
def action_feature_vector_for_state(
    ...
    decisive_attack_margin = decisive_attack_margin_for_state(
        planets,
        fleets,
        player,
        source,
    ...
    features.extend([float(decisive_attack_margin)])
    return features
