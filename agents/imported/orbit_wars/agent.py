...
# Make the sibling ``orbit_lite`` package importable
try:
...
for extra_path in [
    "/kaggle/input/producer-orbit-wars-utils",
    "/kaggle/input/datasets/slawekbiel/producer-orbit-wars-utils",
]:
...
from orbit_wars_orbit_lite.geometry import fleet_speed
from orbit_wars_orbit_lite.intercept_aim import intercept_angle
from orbit_wars_orbit_lite.movement import MovementConfig, PlanetMovement
from orbit_wars_orbit_lite.movement_step import (
...
from orbit_wars_orbit_lite.obs import parse_obs
from orbit_wars_orbit_lite.distance_cache import build_distance_cache
from orbit_wars_orbit_lite.planner_core import (
    _candidate_indices,
    _empty_entries,
    _greedy_select,
    _plan_regroup,
...
from orbit_wars_orbit_lite.adapter import single_obs_to_tensor, sparse_action_row_to_moves
...
class ProducerLiteConfig:
    """Behaviour knobs - tuned for competitive play."""
    ...
    roi_threshold: float = 1.40              # fire if score > this
...
# Dynamic adjustment — continuous interpolation of ROI and Waves
...
def run_turn(obs_tensors: dict, *, config: ProducerLiteConfig, player_count: int, memory) -> dict:
    ...
    # Dynamic adjustment of config based on game state
...
CONFIG_2P = ProducerLiteConfig()  # ROI=1.40, horizon=18, waves=6, beta=2.2

CONFIG_3P = dataclasses.replace(
    ProducerLiteConfig(),
    horizon=15,
